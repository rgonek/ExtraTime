...
public sealed class BotSeederTests : HandlerTestBase
{
    ...
    public async Task SeedDefaultBotsAsync_WhenNoBotsExist_SeedsAllDefaultBotsIncludingPhase95()
    {
        ...
        await Assert.That(_bots.Count).IsEqualTo(15);
        await Assert.That(_users.Count).IsEqualTo(15);
        await Assert.That(_bots.All(bot => bot.Strategy == BotStrategy.StatsAnalyst || bot.Configuration is null)).IsTrue();
