...
public sealed class UnderdogSupporterStrategyTests
{
    private readonly UnderdogSupporterStrategy _strategy = new();

    [Test]
    public async Task GeneratePrediction_IdentifiesUnderdog_FavorsAwayTeam()
    {
        // Arrange
        ...
        // Assert - Away score should be greater than or equal to home score
        await Assert.That(awayScore).IsGreaterThanOrEqualTo(homeScore);
    }
    ...
    public async Task GeneratePrediction_AwayScore_IsAtLeast1()
    {
        ...
        // Assert - Away score should always be at least 1
        await Assert.That(awayScores.Min()).IsGreaterThanOrEqualTo(1);
    }

    [Test]
    public async Task GeneratePrediction_HomeScore_IsLessThanOrEqualToAway()
    {
        // Arrange
        var match = CreateTestMatch();
        ...
        // Assert - Home score should always be less than or equal to away score
        foreach (var (home, away) in predictions)
        {
            await Assert.That(home).IsLessThanOrEqualTo(away);
        }
    }
    ...
    [Test]
    public async Task GeneratePrediction_HomeScoreRange_Is0To3()
    {
        // Arrange
        var match = CreateTestMatch();
    ...
    [Test]
    public async Task StrategyType_ReturnsUnderdogSupporter()
    {
        // Assert
        await Assert.That(_strategy.StrategyType).IsEqualTo(BotStrategy.UnderdogSupporter);
    }
    ...
    public async Task GeneratePrediction_WithConfiguration_IgnoresConfiguration()
    {
        ...
        // Assert - Should still follow underdog pattern
        await Assert.That(awayScore).IsGreaterThanOrEqualTo(homeScore);
